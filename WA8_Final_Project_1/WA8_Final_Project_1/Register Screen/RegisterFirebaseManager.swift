...
extension RegisterViewController{
        ...
        if let name = name, let email = email, let password = password {
                    Auth.auth().createUser(withEmail: email, password: password, completion: {result, error in
                        if error == nil{
                            ...
                            self.createUserDocument(withEmail: email.lowercased(), name: name)
                        }else{
                            print(error)
                        }
                    })
    ...
    func createUserDocument(withEmail email: String, name: String) {
        // Get a reference to the Firestore database
        let db = Firestore.firestore()

        let userData: [String: Any] = ["name": name, "email": email]
